...
import { ValidationError as IValidationError, ValidationResult, NodeData, ConnectionData } from '../types/workflow';
...
export class ValidationError implements IValidationError {
  ...
  constructor(type: string, message: string, nodeId: string | null = null, severity: 'error' | 'warning' | 'info' = 'error') {
    this.type = type;
...
export class WorkflowValidator {
  private rules: Array<(nodes: NodeData[], connections: ConnectionData[]) => ValidationError[]>;
  ...
  validate(editor: any): ValidationResult {
    ...
    for (const rule of this.rules) {
      try {
        ...
        errors.push(new ValidationError(
          'validation_error',
          `Validation rule failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
          null,
          'error'
        ));
      }
  ...
  validateNodeConfiguration(nodes: NodeData[], _connections: ConnectionData[]): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  validateConnections(nodes: NodeData[], connections: ConnectionData[]): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  validateNodeTypes(nodes: NodeData[], connections: ConnectionData[]): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  validateWorkflowStructure(nodes: NodeData[], connections: ConnectionData[]): ValidationError[] {
    const errors: ValidationError[] = [];
  ...
  validateCircularDependencies(nodes: NodeData[], connections: ConnectionData[]): ValidationError[] {
    const errors: ValidationError[] = [];
    const nodeMap = new Map(nodes.map(n => [n.id, n]));
  ...
  generateSummaryText(errors: ValidationError[], warnings: ValidationError[], info: ValidationError[]): string {
    const parts: string[] = [];
